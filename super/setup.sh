...
clear;clear;clear
  ...
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Welcome To leryyvpn Project Script Installer ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
echo -e "     This Will Quick Setup VPN Server On Your Server"
echo -e "         Auther : ${green}leryyvpn ${NC}${YELLOW}(${NC} ${green}leryyvpn Project ${NC}${YELLOW})${NC}"
echo -e "       Â© Recode By leryyvpn Project ${YELLOW}(${NC} 2023 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 5
###### IZIN SC 
...
data_ip="https://raw.githubusercontent.com/leryyvpn/ipkula/main/ip"
checking_sc() {
  ...
  if [[ $date_list < $useexp ]]; then
    ...
    echo -e "      \033[0;36mTelegram${NC} t.me/leryyvpn"
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/leryyvpn/ipkula/main/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/leryyvpn/ipkula/main/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/leryyvpn/ipkula/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://raw.githubusercontent.com/leryyvpn/super/main/"
...
function password_default() {
    ...
    ============================
    (C) Copyright 2023 By leryyvpn Project
    ============================
"
...
rm -rf /etc/vmess/.vmess.db
    ...
    mkdir -p /etc/leryyvpn/limit/vmess/ip
    mkdir -p /etc/leryyvpn/limit/vless/ip
    mkdir -p /etc/leryyvpn/limit/trojan/ip
    mkdir -p /etc/leryyvpn/limit/ssh/ip
